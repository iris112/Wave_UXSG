...
export function route(connectionType: TConnection, buildType: TBuild, type: TPlatform) {
    return function (req: IncomingMessage, res: ServerResponse) {
        ...
        if (req.url.includes('/?code=')){
            //Grab code and make post request to meetup oauth2
            
            var parsedUrl = req.url.split('=');
            console.log(parsedUrl);
            console.log('code');
            var meetupCode = parsedUrl[parsedUrl.length-1];
            const config = {
                headers: {
            ...
            const requestBody = {
                client_id: '8ipcmqc429s5ia89g9ue6n4fej',
                client_secret:'j3gadn42qs0msse6metkf3mhme',
                grant_type:'authorization_code',
                redirect_uri:'http://localhost:8081/',
                code:meetupCode
            }
            axios.post('https://secure.meetup.com/oauth2/access', requestBody, config)
            .then((result)=>{
                console.log("Result: ");
                console.log(result);
            })
...
function routeStatic(req, res, connectionType: TConnection, buildType: TBuild, platform: TPlatform) {
    ...
    const check = (root: string) => {
        ...
        readFile(path).then((file: Buffer) => {
            res.setHeader('Cache-Control', 'public, max-age=3155760000');
